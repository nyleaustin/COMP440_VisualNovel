...
#game variables
#implement score changes based on amount of rows cleared
#implement TETRIS score reward
var score : int
...
#tilemap variables
var tile_id: int = 0 #derived from atlas id below the code
...
func new_game():
	...
	score = 0
	target_score = 1200
	$HUD.get_node("ScoreLabel2").text = "Target Score: " + str(target_score)
	speed = 1.0
	game_running = true
	steps = [0, 0, 0] #index 0:left, 1:right, 2:down
	$HUD.get_node("GameOverLabel").hide()
	$HUD.get_node("StartButton").hide()
	$HUD.get_node("YouWinLabel").hide()
	#clear board
	clear_piece()
	clear_board()
	clear_panel()
	piece_type = pick_piece()
	piece_atlas = Vector2i(shapes_copy.find(piece_type), 0)
	next_piece_type = pick_piece()
	next_piece_atlas = Vector2i(shapes_copy.find(next_piece_type), 0)
	create_piece()
...
#looks through shapes array and randomly picks piece
func pick_piece():
	var piece
...
func draw_piece(piece, pos, atlas): #piece will be any of the shapes above, pos will be the
	for i in piece:
		#arguments for set_cell(layer you are drawing on, cord of tile and i iterator, tile_id, atlas) 
...
func can_move(dir):
	#check if you can move into space
	var cm = true
	for i in active_piece:
...
func land_piece():
	#removes each segment from the active layer and move it to the board layer
	for i in active_piece:
		erase_cell(active_layer, current_pos + i)
		set_cell(board_layer, current_pos + i, tile_id, piece_atlas)
